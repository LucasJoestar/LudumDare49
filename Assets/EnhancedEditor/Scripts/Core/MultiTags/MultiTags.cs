...
namespace EnhancedEditor
{
    ...
    public static class MultiTags
    {
        ...
        /// <inheritdoc cref="TagGroup.GetDatas()"/>
        public static List<TagData> GetTags(TagGroup _group)
        {
            List<TagData> _datas = _group.GetDatas();
            return _datas;
        }
        ...
        {
            TagData[] _datas = Array.ConvertAll(_tags, (t) =>
            {
                TagData _data = t.GetData();
                return _data;
            });
        ...
        {
            TagData[] _datas = Array.ConvertAll(_tagIDs, (i) =>
            {
                TagData _data = GetTag(i);
                return _data;
            });
        ...
        /// </summary>
        /// <param name="_tags"><see cref="Tag"/>[] to get referencing <see cref="TagData"/>.</param>
        /// <param name="_datas">Buffer to store the datas in (with null entries for each tag that could not be found).</param>
        public static void GetTagsNonAlloc(Tag[] _tags, TagData[] _datas)
        {
            int _length = Mathf.Min(_tags.Length, _datas.Length);
            for (int _i = 0; _i < _length; _i++)
            {
                TagData _data = _tags[_i].GetData();
                _datas[_i] = _data;
            }
        }
        ...
        /// <param name="_tagIDs">IDs to get all associated <see cref="TagData"/>.</param>
        /// <param name="_datas">Buffer to store the datas in (with null entries for each tag that could not be found).</param>
        public static void GetTagsNonAlloc(long[] _tagIDs, TagData[] _datas)
        {
            int _length = Mathf.Min(_tagIDs.Length, _datas.Length);
            for (int _i = 0; _i < _length; _i++)
            {
                TagData _data = GetTag(_tagIDs[_i]);
                _datas[_i] = _data;
            }
        }
        #endregion
