...
namespace EnhancedEditor.Editor
{
	...
	public class PinboardWindow : EditorWindow
    {
        ...
        private readonly EditorColor peerColor = EnhancedEditorGUIUtility.GUIPeerLineColor;
        ...
        private void DrawToolbar()
        {
            ...
            if (GUILayout.Button(createFolderGUI, _buttonStyle, GUILayout.Width(125f)))
            {
                ...
                ArrayUtility.Add(ref Pinboard.PinnedAssets.Folders, _newFolder);
            ...
            {
                EnhancedEditorGUILayout.ToolbarSortOptions(ref selectedSortOption, ref doSortAscending, sortOptionsGUI, GUILayout.Width(130f));
                if (_scope.changed)
                {
                    SortObjects();
                }
        ...
        private bool DrawBackground(Rect _position, PinObject _object, int _index, bool _isFolder)
        {
            ...
            {
                EditorGUI.DrawRect(_position, selectedColor.Get());
            }
            else if (_isHover)
            {
            ...
            {
                EditorGUI.DrawRect(_position, peerColor.Get());
            }
            ...
            if (_event.type == EventType.DragPerform)
            {
                ...
                if (DragAndDrop.GetGenericData(FolderDataType) is Folder[] _folders)
                {
                    ...
                    for (int _i = _folders.Length; _i-- > 0;)
                    {
                        ...
                        {
                            ArrayUtility.Remove(ref _folders, _data);
                        }
                        else if (FindParent(Pinboard.PinnedAssets, _data, out Folder _parent))
                        {
                            ...
                            {
                                ArrayUtility.Remove(ref _childParent.Folders, _destination);
                                ArrayUtility.Add(ref _parent.Folders, _destination);
                            }

                            ArrayUtility.Remove(ref _parent.Folders, _data);
                        }
                    }

                    ArrayUtility.AddRange(ref _destination.Folders, _folders);
                }
                ...
                if (DragAndDrop.GetGenericData(AssetDataType) is Asset[] _assets)
                {
                    foreach (Asset _data in _assets)
                    {
                        ...
                        {
                            ArrayUtility.Remove(ref _parent.Assets, _data);
                        }
                    }

                    ArrayUtility.AddRange(ref _destination.Assets, _assets);
                ...
                if (_objects != null)
                {
                    ...
                    foreach (Object _data in _objects)
                    {
                        ...
                        ArrayUtility.Add(ref _destination.Assets, _asset);
                    }
                    ...
                    void RemoveObsoleteObjects(Folder _folder, ref Object[] _objects)
                    {
                        ...
                        for (int _i = _objects.Length; _i-- > 0;)
                        {
                            ...
                            {
                                ArrayUtility.RemoveAt(ref _objects, _i);
                            }
                        }
                    }
                }
