...
namespace EnhancedEditor.Editor
{
    public class SceneHandler : EditorWindow
    {
        ...
        private void DrawSceneLoader(ref Rect _rect)
        {
            ...
            for (int _i = 0; _i < sceneDatas.Length; _i++)
            {
                ...
                if (_isDoubleColumn)
                {
                    ...
                    {
                        // Peer color background.
                        if (_isPeer)
                            EditorGUI.DrawRect(new Rect(0, _rect.y - SceneHeightMargin, _rect.width, _rect.height + (SceneHeightMargin * 2)), peerColor.Get());
        ...
        public void DrawScene(int _sceneIndex, Rect _tagRect, Rect _labelRect, Rect _tokenRect, Rect _buttonRect, Rect _buttonLabelRect, Rect _loadedRect, Rect _dropdownRect)
        {
            ...
            // Tag button.
            using (var _scope = EnhancedGUI.GUIColor.Scope(_tagRect.Contains(Event.current.mousePosition) ? SuperColor.Silver.Get() : GUI.color))
            {
            ...
            // Scene label.
            using (var _scope = EnhancedGUI.GUIColor.Scope(sceneLabelColor))
            {
            ...
            if (_scene.IsLoaded)
            {
                using (var _scope = EnhancedGUI.GUIColor.Scope(closeSceneColor))
                {
                ...
                using (var _scope = EnhancedGUI.GUIColor.Scope(loadedColor))
                {
            ...
            else
            {
                ...
                using (var _scope = EnhancedGUI.GUIColor.Scope(openSceneColor))
                {
